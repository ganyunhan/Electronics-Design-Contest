...
void MLX90614init(void)
{
  ...
  I2C_Channel.Init.AddressingMode = I2C_ADDRESSINGMODE_7BIT;
  I2C_Channel.Init.DualAddressMode = I2C_DUALADDRESS_DISABLE;
  I2C_Channel.Init.OwnAddress2 = 0;
  I2C_Channel.Init.GeneralCallMode = I2C_GENERALCALL_DISABLE;
  I2C_Channel.Init.NoStretchMode = I2C_NOSTRETCH_DISABLE;
...
void MLX90614_WriteReg(uint8_t devAddr, uint8_t regAddr, uint16_t data) {
	...
	HAL_I2C_Master_Transmit(&I2C_Channel, (devAddr << 1), i2cdata, 4, 0xFFFF);
	HAL_Delay(10);
...
uint16_t MLX90614_ReadReg(uint8_t devAddr, uint8_t regAddr, uint8_t dbg_lvl) {
	...
	HAL_I2C_Mem_Read(&I2C_Channel, (devAddr<<1), regAddr, 1, in_buff, 3, 100);
...
void MLX90614_ScanDevices (void) {
	...
	for ( i = 0; i<128; i++)
		  {
			  result = HAL_I2C_IsDeviceReady(&I2C_Channel, (uint16_t) (i<<1), 2, 2);
			  if (result != HAL_OK)
			  {
				  sprintf(temp_buff, ".");
